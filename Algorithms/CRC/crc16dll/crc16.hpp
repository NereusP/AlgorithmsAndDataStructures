...
#ifdef CRC16DLL_EXPORTS
extern "C" _declspec(dllexport) int CalcCRC16(FILE* fr, CRC16_TYPE crc16_type, uint16_t& crc16value);
extern "C" _declspec(dllexport) int CalcCRC16Ex(FILE* fr, uint16_t& crc16value, uint16_t crc_startvalue, uint16_t polyrepres);
#else
extern "C" _declspec(dllimport) int CalcCRC16(FILE* fr, CRC16_TYPE crc16_type, uint16_t& crc16value);
extern "C" _declspec(dllimport) int CalcCRC16Ex(FILE* fr, uint16_t& crc16value, uint16_t crc_startvalue, uint16_t polyrepres);
#endif
